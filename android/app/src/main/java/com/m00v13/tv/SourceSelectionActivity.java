...
public final class SourceSelectionActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle state){
        ...
        AppSettingsStore appSettings=new AppSettingsStore(this);clickSounds=appSettings.clickSounds();artwork=new ArtworkLoader(this);
    ...
    private View build(String title){
        ScreenProfile sp=ScreenProfile.detect(this); ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);scroll.setSmoothScrollingEnabled(false);scroll.setVerticalScrollBarEnabled(false);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(TvUi.dp(this,sp.sidePaddingDp),TvUi.dp(this,28),TvUi.dp(this,sp.sidePaddingDp),TvUi.dp(this,30));root.setBackgroundColor(TvUi.BG);scroll.addView(root);
    ...
    private View sourceRow(int index){
        ...
        row.setOnFocusChangeListener((v,f)->{row.animate().cancel();row.setScaleX(1f);row.setScaleY(1f);row.setBackground(TvUi.focusBackground(this,f,dp(8)));text.setTextColor(f?TvUi.BLUE:TvUi.WHITE);if(f&&clickSounds)row.playSoundEffect(android.view.SoundEffectConstants.CLICK);});
    ...
    private void loadArtwork(ImageView v){if(media==null||media.artworkUrl==null||media.artworkUrl.isEmpty())return;artwork.load(v,media.artworkUrl,240,artExecutor);}

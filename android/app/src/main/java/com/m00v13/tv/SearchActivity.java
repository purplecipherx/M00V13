...
public final class SearchActivity extends Activity {
    ...
    private ScrollView build() {
        ...
        input.setHint("Movie or show title…"); input.setSingleLine(true); input.setTextColor(Color.WHITE);
        input.setHintTextColor(Color.rgb(180,165,195)); input.setTextSize(19); input.setImeOptions(EditorInfo.IME_ACTION_SEARCH);
        ...
        ip.topMargin=dp(12); ip.bottomMargin=dp(12); root.addView(input, ip);

        searchButton = button(new MetadataStore(this).isConfigured() ? "Find movie / show" : "Search sources without metadata");
        searchButton.setOnClickListener(v -> beginSearch()); root.addView(searchButton, full(58));
        status = text(new MetadataStore(this).isConfigured() ?
            "TMDB will identify the exact title first, then M00V13 will scrape sources." :
            "TMDB is not configured. Raw source search is still available.", 15, false);
    ...
    private void beginSearch() {
        ...
        if(!ms.isConfigured()){ scrapeRaw(q); return; }
        ...
        executor.submit(() -> {
            try {
                ...
                runOnUiThread(() -> { if(dead())return; searchButton.setEnabled(true); showMetadataResults(found); });
    ...
    private void showMetadataResults(List<TmdbClient.Result> found) {
        ...
        if(found.isEmpty()){ status.setText("No matching movie or show found."); return; }
        status.setText("Choose the exact title:");
        for(TmdbClient.Result r: found){
            String type = r.series ? "TV" : "Movie";
            Button b = button(r.title + (r.year.isEmpty()?"":" ("+r.year+")") + "  •  " + type);
            b.setGravity(Gravity.START|Gravity.CENTER_VERTICAL);
            b.setOnClickListener(v -> scrapeMetadataResult(r,b));
            LinearLayout.LayoutParams p=full(68); p.bottomMargin=dp(9); results.addView(b,p);
        }
    }

    private void scrapeMetadataResult(TmdbClient.Result r, Button selected) {
        selected.setEnabled(false); status.setText("Scraping sources for " + r.title + "…");
        ...
        executor.submit(() -> {
            try {
                ...
                runOnUiThread(() -> {
                    loading.hide(); if(dead())return; selected.setEnabled(true);
                    catalog.upsert(card);
                    if(scrape.sources.isEmpty()){ status.setText("Title saved, but no usable sources were found."); return; }
                    new SourceStore(this).put(card.id, scrape.sources);
                    status.setText(scrape.sources.size()+" sources found");
    ...
    private void scrapeRaw(String query) {
        ...
        executor.submit(() -> {
            NativeScraperEngine.SearchResult r = new NativeScraperEngine(this).search(query);
            runOnUiThread(() -> { loading.hide(); if(dead())return; searchButton.setEnabled(true);
                if(r.sources.isEmpty()){ status.setText("No usable sources found."); return; }
                String id="search_"+Integer.toHexString(query.toLowerCase(java.util.Locale.US).hashCode()); new SourceStore(this).put(id,r.sources);

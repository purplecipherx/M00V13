...
public final class SearchActivity extends Activity {
    ...
    private ScrollView build() {
        ...
        status = text(new MetadataStore(this).isConfigured() ? "TMDB will identify the exact title first, then M00V13 will search Tier 1 → Tier 2 → Tier 3." : "TMDB is not configured. Raw tiered source search is still available.", 15, false);
    ...
    private void scrapeMetadataResult(TmdbClient.Result r, Button selected) {
        selected.setEnabled(false); status.setText("Searching Tier 1 for " + r.title + "…"); final LoadingOverlay loading = LoadingOverlay.show(this);
        executor.submit(() -> {
            try {
    ...
    private void scrapeRaw(String query) {
        searchButton.setEnabled(false); status.setText("Searching Tier 1 → Tier 2 → Tier 3…"); final LoadingOverlay loading=LoadingOverlay.show(this);
        executor.submit(() -> {
            NativeScraperEngine.SearchResult r = new TieredSearchEngine(this).search(query); List<SourceOption> ranked = r.sources;

...
public final class LoadingOverlay {
    ...
    private LoadingOverlay(Activity activity,String message) {
        ...
        ImageView image=new ImageView(activity); image.setImageResource(R.drawable.loading_cow_hd); image.setScaleType(ImageView.ScaleType.FIT_CENTER); image.setAdjustViewBounds(false); image.setBackgroundColor(Color.BLACK);
        root.addView(image,new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.MATCH_PARENT));
        FrameLayout shade=new FrameLayout(activity); shade.setBackgroundColor(Color.argb(25,0,0,0)); root.addView(shade,new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.MATCH_PARENT));
